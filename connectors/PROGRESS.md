...
| 4 | Australian OAIC Breaches | DONE (scraper built) | `australian-oaic-connector-spec.md` | 2026-04-08 |
...
| 2026-04-10 | EU GDPR Enforcement Tracker scraper built | `eu_gdpr_enforcement_tracker_connector.py` — Tri-strategy scraping: XHR/JSON endpoint discovery → Selenium HTML → direct requests fallback. Full/incremental/lookup modes. Fine amount parsing (monetary/reprimand/ban/undisclosed), European date format handling, GDPR article citation normalization, 31+ country ISO mapping, sector taxonomy mapping, DPA reference table. 28 unit tests passed. CLI verified. Live test: sandbox blocks JS rendering (Selenium unavailable) — graceful fallback confirmed. Next scraper: Australian OAIC. |
| 2026-04-13 | Australian OAIC NDB scraper built | `australian_oaic_connector.py` — HTML report scraping via requests + BeautifulSoup. Discovers 15 half-yearly NDB reports from publications page. Full/incremental/lookup modes. Table extraction + classification (sector, breach source, cyber incident, individuals affected). Regex-based statistic extraction (notifications, percentages). Sector normalization (ANZSIC → Threatpedia), 3-tier cause taxonomy mapping. Known publish dates for incremental sync. PDF URL discovery. Polite scraping with randomized delays. 20 unit tests passed. CLI verified. **Live test passed: 15 reports discovered, H2 2024 parsed — 5 sector entries, 4 breach sources, 7 cyber incident types, PDF link extracted.** No new dependencies beyond `requests` + `beautifulsoup4`. |
